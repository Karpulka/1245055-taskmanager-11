...
(function () {
  ...
  const renderElements = () => {
    ...
    render(mainControlContainer, menuHtml(), `beforeend`);
    render(mainControlContainer, filterHtml(), `afterend`);

    render(boardContainer, sortHtml(), `afterbegin`);

    render(boardTasksContainer, taskEditHtml(), `beforeend`);

    for (let i = 0; i < TASK_COUNT; i++) {
      render(boardTasksContainer, taskCardHtml(), `beforeend`);
    }

    boardContainer.appendChild(boardTasksContainer);

    render(boardContainer, loadMoreHtml(), `beforeend`);
